...
void Laborator4::Init()
{
	...
	Mesh* mesh = new Mesh("teddyMonster");
	mesh->LoadMesh(RESOURCE_PATH::MODELS + "Primitives", "teddy.obj");
...
void Laborator4::Update(float deltaTimeSeconds)
{
	...
	modelMatrix *= Transform3D::Translate(0.0f, 1.0f, 0.0f);
	modelMatrix *= Transform3D::Scale(0.05f, 0.05f, 0.05f);
	RenderMesh(meshes["teddyMonster"], shaders["VertexNormal"], modelMatrix);
}
